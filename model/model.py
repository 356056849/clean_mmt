...
def sharded_cross_view_inner_product(vid_embds,
  ...
  moe_weights = th.div(moe_weights, norm_weights)  # moe_weights is w_i(c) in the paper
  ...
  for idx, mod in enumerate(subspaces):
    ...
    sims += moe_weights[:, :, idx] * th.matmul(text_embds[mod],
                                               vid_embds[mod].t())
    #sims += th.matmul(text_embds[mod], vid_embds[mod].t())
...
def cpt_cross_corrletaion_matrix(vid_embds,
                                 text_embds,
                                 vid_weights,
                                 text_weights,
                                 subspaces,
  ...
  b = vid_embds[subspaces[0]].size(0)
  dim = vid_embds[subspaces[0]].size(1)
  device = vid_embds[subspaces[0]].device
  num_caps = text_embds[subspaces[0]].size(1)
  m = len(subspaces)

  # unroll separate captions onto first dimension and treat them separately
  cross_corr_mat = th.zeros(dim, dim, device=device)
  ...
  for idx, mod in enumerate(subspaces):
    text_embds[mod] = text_embds[mod].view(b * num_caps, -1)  # shape [b, 512]
    weights_for_single_modality = moe_weights[:, 0, idx].unsqueeze(-1).repeat(1, dim)  # [b, 512]
    weighted_vid_embds = (weights_for_single_modality * vid_embds[mod]).t()  # [512, b]
    cross_corr_mat += th.matmul(weighted_vid_embds, text_embds[mod])
    
  if num_caps > 1:
    # aggregate similarities from different captions
    msg = 'number of captions per video must be: {}'
    raise ValueError(msg.format(num_caps))
